...
// fct to fetch specific price
    ...
    async executeSwap() {
      ...
      console.log(this.burnTx, 'butn tx')
...
async decarb(provider,route) {
  ...
  let tx = await squidContract.callBridgeCall(
    this.burnTx.value,
    this.selectedTokenOption.address,
    this.burnTx.value,
    137,
    bctContractAddress,
    polygonContractAddress,
    { value: this.burnTx.value })
*/

  /*
  const pKarbContract = new ethers.Contract(moonbeamContractAddress, receiverABI, signer)
...
console.log(allowance2,amount,'allowance vs amount')
